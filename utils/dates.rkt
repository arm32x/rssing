...
(define (string->date/rfc3339 rfc3339-string)
  (let* ([iso8601-string (regexp-replace #px"([-+]\\d{2}):(\\d{2})$" rfc3339-string "\\1\\2")]
         [date           (string->date iso8601-string "~Y-~m-~dT~H:~M:~S~z")])
    date))
